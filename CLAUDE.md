...
3. **Call 1** (continuity Choice `same`/`extends`/`new`, `needs_context`, one Noul per capability), sent in the same wave as step 4.
4. Content judgments, by index size (spec D14):
   - **Flat mode** (default while the flat pass fits `router.flat_max_tokens`, 40k tokens ≈ 450 files): one strict Noul per file/doc/table card. One round trip in total; no walk, expansion or final pass.
   - **Walk mode** (above the budget): Jev **directory walk** (lenient), graph expansion, then a strict **final pass**.
5. Selection and budget (≤ 12 pointers), then the note.
...
| `docs/architecture-review.md` | Architecture review against the Jev docs (2026-09-24) and the reasoning behind spec D14–D20 (flat-first routing, per-backend limits, HTTP/2, providers). Read before changing the router's request shape. |
...
- **Keep the judge's state small and every call bounded.** State is the redacted prompt plus a few fields (≈ 2k tokens); candidates go in questions, which Jev judges independently. Requests stay within the backend's limits (07 §4.2) and routes within `router.flat_max_tokens`; the index is judged file by file only when it fits (spec principle 5, D15).
...
- Stack: Python 3.11+, `uv`, `ruff`, `pyright` (strict), `pytest`, pydantic v2, typer, httpx (async, HTTP/2 via `h2`), sqlglot. `typesafe-sdk` is a dev-only dependency (spec D18). The distribution name is proposed as `jev-surfer` (import `surf`, console scripts `surf` and `surf-hook`); this is still an open decision.
