...
| Q-09-13 | Questions are judged independently against a small state, so the 40-question cap (spec principle 5) doesn't bound what jaggedness #5 warns about. Route flat-first (one final-wording Noul per leaf card, same wave as call 1) up to a per-route token budget (~40k tokens ≈ 450 file cards), walking only above it; final pass only for expansion-only candidates | No change until measured | A0 vs A4 on repos under the budget + latency curve at 40/100/300 questions (07 §8.4); then owner restates spec §2 principle 5 and the CLAUDE.md non-negotiable (architecture-review.md §3.1) |
| Q-09-14 | A single abstract `needs_context` Noul separates poorly (jev-skillful measured and removed such a gate; TypeSafe's cookbook uses three action-phrased Nouls) | Keep; add a composite-gate candidate; in flat mode use "no content Noul ≥ τ" as the gate | Eval: `no_context_gate` attribution bucket |
| Q-09-15 | Speculation needs no cancellation: at $0.042/Mtok a discarded L1 costs ~$0.0002 | Same wave, stop awaiting on `same`, close the client; drop the `cancelled` ledger outcome. Keep walk questions in separate requests (call 1 state carries `previous_task`, a distractor for `new` tasks) | Design review |
